...
func (gen *myGenerator) init() error {
	...
	buf.WriteString(fmt.Sprintf("初始化完成. (最大并发=%d)", gen.concurrency))
...
func (gen *myGenerator) Start() bool {
	...
	go func() {
		...
		logger.Infof("停止. (请求数: %d)", gen.callCount)
	}()
	return true
}
